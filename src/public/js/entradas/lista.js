...
dialog_eliminar.accept(async ()=>{
    let eliminado=await api.entradas.eliminar(elimnar_id)
    if(eliminado){
        await Listar();
    }else{
        alert("Error")
...
async function Listar() {
    let data=await api.entradas.lista();
    ...
    data.forEach((item,index) => {
        content+=`
            <tr>
                ...
                <td>
                    <button class="btn-editar" onclick="window.location.href = 'entradas/editar/${item.id}'">
...
async function Nuevo(){
    ...
    if (value) {
        ...
        const agrego=await entradas.agregar(value);
        if(agrego){
            Listar()
...
async function Editar(id){
    const obj= await entradas.buscar(id);
    ...
    if (value) {
        const actualizado=await entradas.editar(value);
        if(actualizado){
            Listar()
...
async function Eliminar(id){
    ...
    if (value) {
        const respuesta=await entradas.eliminar(id);
        if(respuesta){
            Listar()
